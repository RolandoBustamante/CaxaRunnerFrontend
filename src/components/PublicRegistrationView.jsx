          ...
          {participants.map((participant, index) => (
            <div key={index} className={`registration-runner-card ${registrationType === "GROUP" && activeParticipantIndex !== index ? "registration-runner-card-hidden" : ""}`}>
              ...
              <div className="registration-grid registration-grid-4">
                ...
                <label>Fecha de nacimiento<span className="registration-birthdate-control"><input type="text" inputMode="numeric" autoComplete="bday" placeholder="DD/MM/AAAA" value={participant.birthDateText ?? formatBirthDateInput(participant.birthDate)} onChange={(e) => setParticipantBirthDate(index, e.target.value)} onBlur={(e) => setParticipantBirthDate(index, e.target.value, true)} required /><input className="registration-birthdate-picker" type="date" value={participant.birthDate} onChange={(e) => setParticipantBirthDate(index, e.target.value, true)} aria-label="Abrir calendario de fecha de nacimiento" /></span><small className="registration-field-help">Puedes escribir 15/11/1990 o 15111990, o usar el calendario.</small></label>
